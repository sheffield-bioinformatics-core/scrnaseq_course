...
At this point we may want to interrogate our newly defined cell type clusters futher. We can use `FindMarkers` to look for specific genes that are differentially expressed between known clusters
...
Seurat has a great deal of customisability and plotting options. Fort more information look at the following vignette
...
Seurat has in-built functionality to carry out a gene enrichment analysis using another package `enrichr` this must be installed before carrying out this step. Enrichr allows users to check a list of genes, for example the ones found in a cluster, to gene set libraries of known biological function. For more information see https://maayanlab.cloud/Enrichr/#.
