...
<div class="warning">
NOTE: Seurat is in the process of being upgraded from Seurat 4 to Seurat 5. This is a major package upgrade expected to be complete in **October 2023**. For the purposes of this course we will continue with Seurat 4. This workflow will be compatable with the Seurat 5 upgrade. For more information click [here](https://satijalab.org/seurat/articles/get_started_v5).
</div>

# Data Import

We start by importing the sample information for the dataset into R. The `sampleSheet` data frame contains information about each of the samples in the dataset; which GEO Accession ID (GSE) they correspond to, and the disease status of the sample.
...
It is then a two-step process to create a object that `Seurat` can use for analysis. An *object* is an R data structure that Seurat uses to organise our single cell data.
...
# EMILY- REMINDER DO WE NEED THE BELOW NOVELTY SCORING?
...
We can also overlay this information on the plot of `nUMI` vs `nGene` to see if we have been sucessful in removing most of the outlier points.
...
By this stage we have identified, and removed, any potentially unreliable *cells* from our dataset. We still have the issue of many zero counts in the data. In technical terms we say that the data are *sparse*. In presence of these zero values will affect the average expression for a given cell (which is used for normalisation). Genes with too many zero entries are going to add to the noise in our dataset and not be informative. 


```{r}
# Extract the counts from our filtered object
counts <- GetAssayData(object = filtered_data, slot = "counts")

# create a matrix of TRUE/FALSE to indicate whether particular gene is non-zero in a given sample
nonzero <- counts > 0

```
Each row of the `nonzero` contains a series of 0 of 1 depending on whether that gene is non-zero in that sample. Since logical (`TRUE`/`FALSE`) values can also be treated as 0 or 1, the sum of a given row will be how many samples that gene is non-zero in

```{r}
## this gene is zero in all samples
table(nonzero[1,])
sum(nonzero[1,])

## this gene (picked arbitarily) is nonzero in 43 samples
table(nonzero[21,])
sum(nonzero[21,])
```
...
rs[21]
```
We can tabulate the row sums to get an idea of the distribution. From this we can see that a large number of genes are zero in all cells. These clearly correspond to some of the genes that we want to remove.

```{r}
data.frame(RowSum = rs) %>% 
  count(RowSum)
```


We choose to pick genes that are **nonzero in 10 or more cells** and create a new count matrix. However, since `Seurat` requires more complex data than just a counts matrix we have to create a seurat object as before. The meta data for this new object can be the same as our filtered object (since we have just removed *genes* from the dataset and not *cells*).

```{r}
## keep_genes will be TRUE or FALSE depending on if a gene is nonzero in at least 10 samples
keep_genes <-rowSums(nonzero) >= 10

## make new count matrix by selecting only rows where keep_genes is TRUE
...
# EMILY NOTE - CAN CAN DO THIS EASIER BELOW?
#CreateSeuratObject(min.cells = 10)
