...
We can start constructing the `for` loop re-writing the code chunk using *variables* in place of the the directory name and project name
```{r eval=FALSE}
...
If we want to read the data for a particular sample, all that is required is to set the values of `the_data_dir` and `project_name` appropriately.
...
We can now "loop" over the `dirs` variable. This will use a indexed value of `dirs` starting at 1 until all values of `dirs` have been processed. At each *iteration* of the loop we can find out the current value of `dirs` and `samples` using `dirs[i]` and `samples[i]` respectively.

Unlike the previous code chunk we save the result of each `CreateSeuratObject` call to a `list` variable.
...
- Verify that the `merged_data` object
</div>
