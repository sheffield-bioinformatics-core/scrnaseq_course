...
We are going to import the data for the first disease sample - `GSM3872434` The code to read a single sample is given below. This assumes that the output from the *cellranger* tool can be found in the `GSM3872434` directory. This output consists of the files `barcodes.tsv.gz`, `features.tsv.gz` and `matrix.mtx.gz`. As part of the course setup ([`download_GSE132509.R`]("download_GSE132509.R")) we have already renamed the outputs to the convention that `Seurat` is expecting. 
...
#### Exercise
<div class="exercise">
- Use to sampleSheet to determine the directory containing data from the first healthy individual (`PBMMC_1`).
...
- We want to remove genes with lots of 0 values
- Will impact normalisation
