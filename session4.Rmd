...
The `org.Hs.eg.db` database is a genome wide annotation package for human that has many different gene mappings based on Entrez Gene identifiers. These are up to data databases of gene anotation that can be installed like any other biocunductor package
...
In our case we are going to use the gene symbols (rownames in our Seurat object) to extract the gene description and entrez ID from the database. In order to see what we can extract we can run the `columns` function on the annotation database.
